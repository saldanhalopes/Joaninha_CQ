...
import Classes.DAO.UnidadeDAO;
import Classes.DAO.VidrariaDAO;
import Classes.Acesso.VidrariaAcesso;
...
public class FrmVidraria extends javax.swing.JInternalFrame {
    ...
    private void initComponents() {
        ...
        itmExcluirVidraria = new javax.swing.JMenuItem();
        ...
        if (tblVidraria.getColumnModel().getColumnCount() > 0) {
            ...
            tblVidraria.getColumnModel().getColumn(5).setPreferredWidth(150);
            tblVidraria.getColumnModel().getColumn(5).setMaxWidth(150);
        ...
        itmBaixaVidraria.setAccelerator(javax.swing.KeyStroke.getKeyStroke(java.awt.event.KeyEvent.VK_PAGE_DOWN, java.awt.event.InputEvent.ALT_MASK));
        ...
        itmExcluirVidraria.setAccelerator(javax.swing.KeyStroke.getKeyStroke(java.awt.event.KeyEvent.VK_DELETE, java.awt.event.InputEvent.ALT_MASK));
        itmExcluirVidraria.setText("Excluir");
        itmExcluirVidraria.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                itmExcluirVidrariaActionPerformed(evt);
            }
        });
        mnuEditarVidraria.add(itmExcluirVidraria);
    ...
    private void tblVidrariaKeyReleased(java.awt.event.KeyEvent evt) {//GEN-FIRST:event_tblVidrariaKeyReleased
        if (tblVidraria.getSelectedRow() != -1) {
            if (evt.getKeyCode() == KeyEvent.VK_DELETE) {
                excluirVidraria((Integer) tblVidraria.getValueAt(tblVidraria.getSelectedRow(), 0));
            }
        }
    ...
    private void btnImprimirRelatorioActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_btnImprimirRelatorioActionPerformed
        int dialogResult = JOptionPane.showConfirmDialog(null,
                ...
                + ("".equals(cmbPesquisarSetor.getSelectedItem().toString())
                        ? "Todos" : cmbPesquisarSetor.getSelectedItem().toString())
                + "\nStatus: " + cmbPesquisarStatus.getSelectedItem().toString(),
                "Aviso", JOptionPane.YES_NO_OPTION);
        if (dialogResult == 0) {
            VidrariaDAO.printVidraria(System.getProperty("user"),
                    cmbPesquisarSetor.getSelectedItem().toString(),
                    (Integer) cmbPesquisarStatus.getSelectedIndex());
        }
    }//GEN-LAST:event_btnImprimirRelatorioActionPerformed

    private void itmExcluirVidrariaActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_itmExcluirVidrariaActionPerformed
        if (tblVidraria.getSelectedRow() != -1) {
            excluirVidraria((Integer) tblVidraria.getValueAt(tblVidraria.getSelectedRow(), 0));
        }
    }//GEN-LAST:event_itmExcluirVidrariaActionPerformed
    ...
    public static javax.swing.JMenuItem itmExcluirVidraria;
    ...
    public final void readVidrarias() {
        ...
        Integer limit = Integer.valueOf(cmbLimitPesquisarVidraria.getSelectedItem().toString());
        ...
        for (Vidraria vidr : vidrDao.readVidraria(limit, setor, status)) {
            model.addRow(new Object[]{
                ...
                vidr.getCertificado(),
                DataHora.getStringDate(vidr.getData_entrada()),
                DataHora.getStringDateTime(vidr.getData_saida()),
    ...
    private void baixaVidraria(int id) {
        VidrariaAcesso Acesso = new VidrariaAcesso();
        if (Acesso.verificarBaixaVidraria()) {
            Vidraria vidr = new Vidraria();
            VidrariaDAO vidrDAO = new VidrariaDAO();
            try {
                int dialogResult = JOptionPane.showConfirmDialog(null, 
                        "Realmente Deseja dar Baixa Nessa Vidraria?", "Aviso", JOptionPane.YES_NO_OPTION);
                if (dialogResult == 0) {
                    vidr.setVidraria_id(id);
    ...
    private void excluirVidraria(int id) {
        VidrariaAcesso Acesso = new VidrariaAcesso();
        if (Acesso.verificarDeletarDadosVidraria()) {
            ...
            try {
                int dialogResult = JOptionPane.showConfirmDialog(null,
                        "Realmente Deseja Excluir essa Vidraria?", "Aviso", JOptionPane.YES_NO_OPTION);
                if (dialogResult == 0) {
                    vidr.setVidraria_id(id);
                    vidrDAO.deleteVidraria(vidr);
                }
            } catch (Exception e) {
                JOptionPane.showMessageDialog(null, "Erro ao atulalizar dados: " + e);
            } finally {
