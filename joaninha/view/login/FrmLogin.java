 ...
 */
package joaninha.view.login;

import joaninha.dao.LoginJpaController;
import joaninha.view.principal.FrmPrincipal;
...
import joaninha.dao.LogLoginJpaController;
import joaninha.dao.LogOffJpaController;
...
//        }
    ...
    private void userSenhaInvalidos(String texto) {
        try {
            ...
            user.setFailedAccessCount(1);
            user.setUserName(txtUsuario.getText().toLowerCase());
            userDao.edit(user);
    ...
    private void acessarSistema() {
        ...
        LogLoginJpaController logLoginDao = new LogLoginJpaController();
        user.setUserName(txtUsuario.getText().toLowerCase());
        user.setPassword(Cript.md5(new String(txtSenha.getPassword())));
            ...
            if ("".equals(txtUsuario.getText().toLowerCase()) || "".equals(new String(txtSenha.getPassword()))) {
                ...
                switch (loginDao.checkLogin(user)) {
                    case 0:
                        ...
                        break;
                    case 1:
                        LogOffJpaController logOffDao = new LogOffJpaController();
                        int falhas = userDao.getFailedAccessCount(user);
                        if (falhas > 0) {
                            JOptionPane.showMessageDialog(this, "Número de falhas na tentativas de Login: "
                                    + falhas,
                                    "Aviso", JOptionPane.WARNING_MESSAGE);
                        }
                        if (logOffDao.booleanLogOff(1)) {
                            ...
                            if (logResult == 0) {
                                user.setComputador(Sistema.getComputador());
                                user.setLastLogin(DataHora.getTimestampDate(new Date()));
                                user.setUserComputador(System.getProperty("user.name"));
                                user.setFailedAccessCount(0);
                                userDao.edit(user);
                                LogLogin loglogin = new LogLogin();
                                loglogin.setComputador(Sistema.getComputador());
                                loglogin.setStatus("Login");
                                loglogin.setDataHora(DataHora.getTimestampDate(new Date()));
                                loglogin.setUserComputador(System.getProperty("user.name"));
                                loglogin.setUserName(user.getUserName());
                                logLoginDao.create(loglogin);
